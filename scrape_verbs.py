...
def generate_verb_forms(dictionary_forms):
    ...
    for dictionary_form in dictionary_forms:
        ...
        if(not os.path.isfile(verb_properties_filepath)
                and not os.path.isfile(verb_forms_filepath)):
            print "Generating verb information {}".format(dictionary_form)
            verb_form_webpage = requests.get(verb_form_url)

            parser = html.HTMLParser(encoding='utf-8')
            tree_root = html.fromstring(
                verb_form_webpage.content, parser=parser)
            ...
            for selected_element in selected_elements:
                ...
                verb_class_row = [verb_class_column,
                                  verb_class, verb_class_kanji]
                ...
                infinitive_row = [infinitive_column,
                                  infinitive, infinitive_kanji]
...
def main():
    ...
    dictionary_form_verbs = [dictionary_form
                             for dictionary_form
                             in dictionary_form_verbs
                             if dictionary_form not in verbs_to_skip]
